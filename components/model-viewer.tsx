...
// Custom controls component for manual rotation and movement
function ManualControls({
  ...
  return (
    <div className="flex items-center gap-2">
      {/* Up button */}
      <Button variant="outline" size="sm" className="bg-white" onClick={onRotateUp}>
        <ChevronUp className="h-4 w-4" />
      </Button>

      {/* Left, Right, Zoom buttons */}
      <div className="flex gap-1">
        <Button variant="outline" size="sm" className="bg-white" onClick={onRotateLeft}>
          <ChevronLeft className="h-4 w-4" />
        </Button>
        <Button variant="outline" size="sm" className="bg-white" onClick={onZoomOut}>
          <ZoomOut className="h-4 w-4" />
        </Button>
        <Button variant="outline" size="sm" className="bg-white" onClick={onZoomIn}>
          <ZoomIn className="h-4 w-4" />
        </Button>
        <Button variant="outline" size="sm" className="bg-white" onClick={onRotateRight}>
          <ChevronRight className="h-4 w-4" />
        </Button>
      </div>

      {/* Down button */}
      <Button variant="outline" size="sm" className="bg-white" onClick={onRotateDown}>
...
// Component that handles camera rotation inside Canvas
function CameraController({ onRotateLeft, onRotateRight }: { onRotateLeft: () => void; onRotateRight: () => void }) {
  ...
  useEffect(() => {
    ...
    const leftHandler = () => handleRotateLeft()
    const rightHandler = () => handleRotateRight()

    // Store references to the functions
    ;(window as any).rotateLeft = leftHandler
    ;(window as any).rotateRight = rightHandler

    return () => {
      delete (window as any).rotateLeft
      delete (window as any).rotateRight
    }
  }, [camera])
...
export default function ModelViewer({ imageUrl }: ModelViewerProps) {
  ...
  const handleRotateLeft = () => {
    setRotation(prev => ({ ...prev, y: prev.y + Math.PI / 8 }))
  }
  ...
  const handleRotateUp = () => {
    setRotation(prev => ({ ...prev, x: prev.x - Math.PI / 8 }))
  }
